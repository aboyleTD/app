...
const DropDownSelect = (props: Props) => {
    ...
    return (
        <button
            ...
            className="relative cursor-pointer rounded-full border border-light-gray bg-white px-5 py-2 text-left transition-all duration-200 hover:border-gray focus:border-dark-gray disabled:cursor-default disabled:border-light-gray disabled:bg-ghost-white disabled:text-gray">
            {getSelectedKey()}
            ...
            {isExpanded ? (
                <div className="absolute left-5 right-5 top-full z-50 max-h-40 translate-y-1.5 cursor-pointer overflow-hidden overflow-y-scroll rounded-lg border border-light-gray bg-white">
                    {props.options.map(
